...
def test_centwave(path):
    ...
    pm = utils.loadPeakMap(path("data", "test_mini.mzXML"))
    table = utils.runCentwave(pm,
                           ppm=3,
                           peakwidth=(8, 13),
                           snthresh=40,
                           prefilter=(8, 10000),
...
def testmatched_filter(path):
    ...
    table = utils.runMatchedFilters(pm,
    ...
    #table.store("features.table", forceOverwrite=True)

@pytest.mark.slow
def test_metabo(path):
    pm = utils.loadPeakMap(path("data", "test_mini.mzXML"))
    table = utils.runMetaboFeatureFinder(pm, common_noise_threshold_int=5000.0,
