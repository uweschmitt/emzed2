...
def testNoIntegration():
    ...
    integrator = dict(emzed.configs.peakIntegrators)["no_integration"]
...
def testPeakIntegration():
    ...
    integrator = dict(emzed.configs.peakIntegrators)["asym_gauss"]
    _, _, params = run(integrator, 1.19e5, 7.2891e3)

    integrator = dict(emzed.configs.peakIntegrators)["emg_exact"]

    run(integrator,  154542.79, 7.43274e3)

    integrator = dict(emzed.configs.peakIntegrators)["trapez"]

    run(integrator,  120481.9, 0.0)

    integrator = dict(emzed.configs.peakIntegrators)["std"]
